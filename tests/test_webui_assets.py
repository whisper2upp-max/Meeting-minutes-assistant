...
def test_release_builds_embed_version_metadata():
    ...
    for content in (workflow, macos_script):
        assert "Set :CFBundleShortVersionString 0.1.2" in content
        assert "Set :CFBundleVersion 0.1.2" in content
    assert "filevers=(0, 1, 2, 0)" in windows_version
    assert "prodvers=(0, 1, 2, 0)" in windows_version
    assert "StringStruct('FileVersion', '0.1.2')" in windows_version
    assert "StringStruct('ProductVersion', '0.1.2')" in windows_version
